...
from lexical_analyzer.my_token import Token
...
class Parser:
    ...
    def advance(self):
        ...
        if self.debug and self.current_token:
            print(f"Advanced to the next token: {self.current_token}")
    ...
    def declaration(self):
        ...
        if self.current_token and self.current_token.lexeme in ["integer", "boolean", "real"]:
            ...
            self.error("Expected a valid declaration")
    def ids(self):
        """
    ...
    def statement(self):
        """
        R15. <Statement> ::= <Assign> | <If> | <Return> | <Print> | <Scan> | <While>
        """
        self.print_production("<Statment> -> <Assign> | <If> | <Return> | <Print> | <Scan> | <While>")

        if self.current_token.token_type == "IDENTIFIER":
        # Handle assignment statement
            self.print_production("<Statement> -> <Assignment>")
            self.assignment()
    
        elif self.current_token.token_type == "KEYWORD" and self.current_token.lexeme == "while":
            # Handle while statement
            self.print_production("<Statement> -> <While Statement>")
            self.while_statement()
    
        elif self.current_token.token_type == "KEYWORD" and self.current_token.lexeme == "if":
            # Handle if statement
            self.print_production("<Statement> -> <If Statement>")
            self.if_statement()
    
        elif self.current_token.token_type == "KEYWORD" and self.current_token.lexeme == "scan":
            # Handle scan statement
            self.print_production("<Statement> -> <Scan Statement>")
            self.scan_statement()
    
        elif self.current_token.token_type == "KEYWORD" and self.current_token.lexeme == "print":
            # Handle print statement
            self.print_production("<Statement> -> <Print Statement>")
    ...
    def factor(self):
        ...
        if self.current_token.lexeme == "-":
            self.match(lexeme="=")
            self.primary()
        elif self.current_token.token_type in ["IDENTIFIER", "INTEGER", "REAL","SEPERATOR" ]:
            self.primary()
        else:
            self.error("Enxpected a valid factor")
    ...
    def primary(self):
        ...
        # Check for IDs
        if self.current_token and self.current_token.token_type == "IDENTIFIER":
            self.match(TOKEN_IDENTIFIER)
    ...
    def assignment(self):
        """
        Implements <Assignment> rule:
        <Assignment> ::= <Identifier> = <Expression> ;
        """
        self.print_production("<Assignment> -> <Identifier> = <Expression> ;")
    
        # Match the identifier
        if self.current_token.token_type == "IDENTIFIER":
            self.match(TOKEN_IDENTIFIER)
        else:
            self.error("Expected an identifier for assignment")
    ...
    def if_statement(self):
        """
        Implements <If Statement> rule:
        <If Statement> ::= if ( <Condition> ) <Statement> else <Statement>
        """
        self.print_production("<If Statement> -> if ( <Condition> ) <Statement> else <Statement>")
        ...
        if self.current_token and self.current_token.lexeme == "else":
            ...
            self.statement()  # Parse the statement after 'else'
        else:
            self.error("Expected 'else' after 'if' statement")
    def block(self):
        """
        Implements <Block> rule:
        <Block> ::= { <Statement List> }
        """
        self.print_production("<Block> -> { <Statement List> }")
        ...
        if self.current_token.lexeme == "}":
            ...
            self.error("Expected '}' to end block")